...
function applyCoupleInfo() {
    ...
    if (brideSection) {
        ...
        if (brideDescElement && bride.description) {
            brideDescElement.innerHTML = bride.description;
        }
    ...
    if (groomSection) {
        ...
        if (groomDescElement && groom.description) {
            groomDescElement.innerHTML = groom.description;
        }
...
// Function để mở Facebook profile với hỗ trợ mobile app
function openFacebookProfile(facebookUrl, personName) {
    // Kiểm tra nếu đang trên mobile device
    const isMobile = /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    
    if (isMobile) {
        // Trên mobile, cố gắng mở Facebook app trước
        try {
            // Trích xuất username/profile ID từ URL
            let profileId = extractFacebookProfileId(facebookUrl);
            
            if (profileId) {
                // URL scheme để mở Facebook app
                const facebookAppUrl = `fb://profile/${profileId}`;
                const fallbackUrl = facebookUrl;
                
                // Hiển thị popup hỏi người dùng
                const userChoice = confirm(
                    `Bạn muốn xem Facebook của ${personName} bằng:\n\n` +
                    `👍 Bấm OK để mở bằng ứng dụng Facebook\n` +
                    `🌐 Bấm Cancel để mở bằng trình duyệt`
                );
                
                if (userChoice) {
                    // Thử mở Facebook app
                    const appOpened = tryOpenFacebookApp(facebookAppUrl, fallbackUrl);
                    if (!appOpened) {
                        // Nếu không mở được app, fallback về browser
                        setTimeout(() => {
                ...
                // Không trích xuất được profile ID, mở bằng browser
                window.open(facebookUrl, '_blank');
            }
        } catch (error) {
            console.log('Error opening Facebook profile:', error);
            window.open(facebookUrl, '_blank');
        }
    } else {
        // Trên desktop, mở bằng browser như bình thường
        window.open(facebookUrl, '_blank');
    }
}

// Function để trích xuất Facebook profile ID từ URL
function extractFacebookProfileId(facebookUrl) {
    try {
        // Các pattern phổ biến của Facebook URL:
        ...
        console.log('Error parsing Facebook URL:', error);
        return null;
    }
}

// Function để thử mở Facebook app
function tryOpenFacebookApp(appUrl, fallbackUrl) {
    try {
        // Tạo invisible iframe để thử mở app
        const iframe = document.createElement('iframe');
        iframe.style.display = 'none';
        iframe.src = appUrl;
        document.body.appendChild(iframe);
        
        // Remove iframe sau 2 giây
        setTimeout(() => {
            document.body.removeChild(iframe);
        }, 2000);
        
        // Detect nếu app không mở được
        const startTime = Date.now();
        setTimeout(() => {
            const timeElapsed = Date.now() - startTime;
            // Nếu time elapsed > 1.5s, có khả năng app đã mở
            if (timeElapsed < 1500) {
                // App có khả năng không mở được, fallback
                window.open(fallbackUrl, '_blank');
            }
        }, 1500);
        ...
        console.log('Error trying to open Facebook app:', error);
        return false;
    }
} 